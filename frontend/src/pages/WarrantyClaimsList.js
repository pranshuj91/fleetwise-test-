...
const WarrantyClaimsList = () => {
  ...
  const fetchClaims = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/warranty/claims${params}`, {
      ...
      if (response.ok) {
        ...
        setStats({
          total: data.length,
          submitted: data.filter(c => c.status === 'submitted' || c.status === 'under_review').length,
          approved: data.filter(c => c.status === 'approved' || c.status === 'paid').length,
          totalRecovered: data
            .filter(c => c.status === 'approved' || c.status === 'paid')
            .reduce((sum, c) => sum + (c.approved_amount || c.total_claim_amount), 0)
        });
      }
    } catch (error) {
  ...
  const downloadClaimPDF = async (claimId, claimNumber) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/warranty/claims/${claimId}/generate-pdf`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
          ...
          <Card className="bg-gradient-to-r from-[#124481] to-[#1E7083] text-white">
            <CardContent className="pt-6">
              <div className="text-center">
                ...
                <p className="text-3xl font-bold">${stats.totalRecovered.toFixed(2)}</p>
              </div>
            </CardContent>
          </Card>
        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-semibold">
                            ${claim.parts_claimed.reduce((sum, p) => sum + (p.quantity * p.cost), 0).toFixed(2)}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500">Labor</p>
                          <p className="font-semibold">
                            ${(claim.labor_hours_claimed * claim.labor_rate).toFixed(2)}
                          </p>
                        </div>
                        <div>
