...
const ETABoard = () => {
  ...
  const fetchDelays = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/office/eta-board`,
        {
          headers: {
  ...
  const updateETA = async (projectId) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/${projectId}/update-eta`,
        {
          method: 'POST',
          headers: {
  ...
  const markDelayed = async (projectId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/${projectId}/mark-delayed`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
