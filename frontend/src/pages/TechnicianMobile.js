...
const TechnicianMobile = () => {
  ...
  const fetchMyJobs = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/my-jobs`,
        {
          headers: {
  ...
  const startJob = async (jobId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/${jobId}/start-time`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const endJob = async (jobId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/${jobId}/end-time`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
