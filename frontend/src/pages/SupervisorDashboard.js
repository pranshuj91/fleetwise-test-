...
const SupervisorDashboard = () => {
  ...
  const fetchShopFloorStatus = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/shop-floor/status`, {
  ...
  const fetchSupervisedTasks = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/supervised`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const reviewTask = async (taskId, approved, feedback = null) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/tasks/${taskId}/review?${params}`,
        {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
