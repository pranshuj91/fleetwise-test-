...
const WarrantyAutoDetect = () => {
  ...
  const fetchWarrantyOpportunities = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/warranty/auto-detect`,
        {
  ...
  const fileWarrantyClaim = async (projectId) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/warranty/auto-file/${projectId}`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
  ...
  const dismissOpportunity = async (projectId) => {
    try {
      await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/warranty/dismiss/${projectId}`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
