...
const PartsQueue = () => {
  ...
  const fetchPartsRequests = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/parts-requests?status=${filter}`,
        {
          headers: {
  ...
  const updateRequestStatus = async (requestId, status, eta = null) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/parts-requests/${requestId}/status`,
        {
          method: 'POST',
          headers: {
