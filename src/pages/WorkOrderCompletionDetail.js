...
const WorkOrderCompletionDetail = () => {
  ...
  const fetchCompletion = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/completions/${completionId}`,
        {
          headers: {
  ...
  const addManagerNote = async () => {
    ...
    try {
      setAddingNote(true);
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/completions/${completionId}/notes`,
        {
          method: 'POST',
          headers: {
  ...
  const approveWorkOrder = async () => {
    try {
      setApproving(true);
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/work-orders/completions/${completionId}/approve`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
