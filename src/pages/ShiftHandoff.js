...
const ShiftHandoff = () => {
  ...
  const fetchLatestHandoff = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/shift-handoff/latest`, {
  ...
  const fetchHandoffHistory = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/shift-handoff/history?days=7`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const acknowledgeHandoff = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/shift-handoff/${latestHandoff.id}/acknowledge`,
        {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
  ...
  const submitHandoff = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/shift-handoff`, {
        method: 'POST',
        headers: {
