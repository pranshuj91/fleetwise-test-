...
const TechnicianMobile = () => {
  ...
  const fetchMyTasks = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/my-tasks`, {
  ...
  const updateTaskStatus = async (taskId, status) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/${taskId}/update`, {
        method: 'PUT',
        headers: {
  ...
  const toggleChecklistItem = async (index) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/tasks/${selectedTask.id}/update`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
