...
const TruckList = () => {
  ...
  useEffect(() => {
    const fetchTrucks = async () => {
      try {
        const response = await truckAPI.list(user?.company_id);
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      filtered = trucks.filter(truck => 
        ...
        truck.identity.make?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        truck.identity.model?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
    
    // Sort by unit number
    const sorted = [...filtered].sort((a, b) => {
      const unitA = a.identity.unit_id || a.identity.truck_number || '';
      const unitB = b.identity.unit_id || b.identity.truck_number || '';
        ...
        <Card className="mb-6">
          <CardContent className="pt-6">
            <div className="relative">
              ...
              <Input
                placeholder="Search by VIN, truck number, make, or model..."
                value={searchQuery}
                ...
                <CardContent className="py-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-6 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600 mt-1">
                          VIN: <span className="font-mono">{truck.identity.vin}</span>
                        </div>
                      </div>
                      ...
                      {truck.identity.odometer_mi && (
                        <div className="w-32 text-right flex-shrink-0">
                          ...
                          <div className="font-medium text-gray-900">
                            {truck.identity.odometer_mi.toLocaleString()} mi
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex items-center space-x-4 ml-6">
                      ...
                      <Badge className="bg-[#289790]">{truck.data_completeness}% Complete</Badge>
                    </div>
                  </div>
                </CardContent>
              </Card>
