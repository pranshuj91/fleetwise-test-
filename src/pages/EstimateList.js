...
const EstimateList = () => {
  ...
  const calculateStats = () => {
    const total = estimates.reduce((sum, est) => sum + est.estimated_total, 0);
    const approved = estimates.filter(e => e.status === 'approved').reduce((sum, est) => sum + est.estimated_total, 0);
    const pending = estimates.filter(e => e.status === 'sent').reduce((sum, est) => sum + est.estimated_total, 0);
    
    return { total, approved, pending, count: estimates.length };
  };
