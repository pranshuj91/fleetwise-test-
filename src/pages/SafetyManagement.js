...
const SafetyManagement = () => {
  ...
  const fetchIncidents = async () => {
    try {
      setLoading(true);
      let url = `${process.env.REACT_APP_BACKEND_URL}/api/safety/incidents?`;
  ...
  const reportIncident = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/safety/incidents`,
        {
          method: 'POST',
          headers: {
  ...
  const resolveIncident = async (incidentId) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/safety/incidents/${incidentId}/resolve`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
