...
const WarrantyClaimCreate = () => {
  ...
  const fetchProjectAndAnalysis = async () => {
    try {
      ...
      // Fetch project
      const projectRes = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/projects/${projectId}`, {
      ...
      const analysisRes = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/warranty/list`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/warranty/claims`, {
        method: 'POST',
        headers: {
